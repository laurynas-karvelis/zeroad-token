...
describe("result cache internals", () => {
    const good = { subscriber: true, plan: 1, expiresAt: 4_000_000_000 };
    const bad = { subscriber: false, reason: REJECTED.FORGED };
    ...
    test("stays bounded under sustained unique traffic", () => {
        ...
        for (let index = 0; index < 5000; index++)
            cache.set(`key-${index}`, good, index);
