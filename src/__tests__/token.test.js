...
import { AUTHORITY_SIGNATURE_OFFSET, credentialMessage, EPHEMERAL_PUBLIC_KEY_OFFSET, HOSTNAME_SIGNATURE_OFFSET, hostnameMessage, NONCE_OFFSET, readToken, TOKEN_BYTES, TOKEN_CHARACTERS, } from "../token";
...
describe("wire format", () => {
    ...
    test("never carries the hostname, so there is nothing to compare or spoof", () => {
        ...
        expect(Buffer.from(bytes).includes(hostname)).toBe(false);
    });
});
describe("readToken", () => {
    ...
    test("rejects an unknown plan byte rather than guessing", () => {
        expect(readToken(mintToken(createAuthority(), "example.com", { plan: 7 }))).toBe("malformed");
        expect(readToken(mintToken(createAuthority(), "example.com", { plan: 0 }))).toBe("malformed");
    });
