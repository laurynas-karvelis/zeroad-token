...
import { AUTHORITY_SIGNATURE_OFFSET, HOSTNAME_SIGNATURE_OFFSET, NONCE_OFFSET, readToken, TOKEN_BYTES, TOKEN_CHARACTERS, } from "../token";
import { bindToHostname, createAuthority, issueCredential, mintToken } from "./__fixtures__/authority";
const HOSTNAME = "example.com";
function build(authority, overrides = {}) {
...
const decode = (token) => new Uint8Array(Buffer.from(token, "base64url"));
...
describe("the runtime fallback actually works", () => {
    ...
    test("a full token verifies through WebCrypto, the path edge runtimes take", async () => {
        const web = webCryptoVerifier();
        if (!web)
            throw new Error("WebCrypto Ed25519 unavailable in this runtime");
        useVerifier(web);
    ...
    test.each([
        ["an empty module", {}],
        ["only createPublicKey", { createPublicKey: (() => { }) }],
        ["only verify", { verify: (() => { }) }],
        ["non-callable properties", { createPublicKey: {}, verify: {} }],
...
describe("fuzzing", () => {
    ...
    test("random bytes never parse as a well-formed token", () => {
        ...
        for (let attempt = 0; attempt < 5000; attempt++) {
            if (typeof readToken(encode(new Uint8Array(randomBytes(TOKEN_BYTES)))) !== "string")
                parsed++;
        }
