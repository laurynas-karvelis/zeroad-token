...
function decodeStandardBase64(base64) {
 ...
 */
export async function nodeCryptoVerifier() {
    try {
...
export function verifierFromNodeCrypto(nodeCrypto) {
    const { createPublicKey, verify } = nodeCrypto;
    if (typeof createPublicKey !== "function" || typeof verify !== "function")
        return undefined;
...
export function webCryptoVerifier() {
    ...
    return async (message, signature, rawPublicKey) => {
        const key = await crypto.subtle.importKey("raw", rawPublicKey, { name: "Ed25519" }, false, [
            "verify",
        ]);
 ...
 */
export async function verifyEd25519(message, signature, rawPublicKey) {
    if (!verifier) {
 ...
 */
export function useVerifier(override) {
    verifier = override;
